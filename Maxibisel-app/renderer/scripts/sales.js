...
export const salesManager = {
    ...
    sortLensesById(lensesArray) {
        if (!Array.isArray(lensesArray) || typeof lensesArray.sort !== "function") return;
        try {
    ...
    updateSelectedCounter() {
        ...
        if (countElement && Array.isArray(this.state.selectedLenses)) {
            const totalItems = this.state.selectedLenses.reduce(
                (sum, lens) => sum + (lens.quantity || 0), 0
            );
    ...
    productoCoincideConBusqueda(lens, searchTerm) {
        ...
        if (lens.sphere && lens.sphere !== 'N' && lens.sphere !== 'N/A') {
            ...
            if (esferaNormalizada === searchTerm || esferaNormalizada.includes(searchTerm)) return true;
        ...
        if (lens.cylinder && lens.cylinder !== '-' && lens.cylinder !== 'N/A') {
            ...
            if (cilindroNormalizado === searchTerm || cilindroNormalizado.includes(searchTerm)) return true;
        ...
        if (lens.addition && lens.addition !== '-' && lens.addition !== 'N/A') {
            ...
            if (adicionNormalizada === searchTerm || adicionNormalizada.includes(searchTerm)) return true;
    ...
    renderSearchResults() {
        ...
        resultsContainer.innerHTML = this.state.searchResults.map((lens) => {
    ...
    renderSelectedLenses() {
        ...
        selectedContainer.innerHTML = this.state.selectedLenses.map((lens) => {
            const specsText = formatLensSpecs(lens);
            const titleHTML = getLensTitle(lens.name);
            return `
    ...
    formatLensSpecsForLog(lens) {
        if (!lens) return "";
        const parts = [];
        if (lens.sphere && lens.sphere !== "N/A" && lens.sphere !== "" && lens.sphere !== "N") parts.push(`Esf: ${lens.sphere}`);
        if (lens.cylinder && lens.cylinder !== "N/A" && lens.cylinder !== "" && lens.cylinder !== "-") parts.push(`Cil: ${lens.cylinder}`);
        if (lens.addition && lens.addition !== "N/A" && lens.addition !== "" && lens.addition !== "-") parts.push(`Add: ${lens.addition}`);
        ...
        try {
            ...
            for (const selectedLens of this.state.selectedLenses) {
                ...
                if (currentStock < quantityToSubtract) {
                    ...
                    advertencias.push(`"${product.name}": Stock insuficiente (Disponible: ${currentStock}, Solicitado: ${quantityToSubtract}, Faltante: ${faltante}). Se registrará como stock negativo.`);
                }
            }
            ...
            for (const selectedLens of this.state.selectedLenses) {
                ...
                if (sumaParciales !== newStock) {
                    throw new Error(`"${product.name}": Inconsistencia en cálculo de stock (${sumaParciales} ≠ ${newStock})`);
                }
                ...
                if (!updatedProduct || updatedProduct.stock === undefined) {
                    throw new Error(`"${product.name}": El backend no devolvió datos válidos de actualización`);
                }

                if (syncHelper && typeof syncHelper.notifyProductSold === "function") {
                    syncHelper.notifyProductSold(selectedLens._id, quantityToSubtract, newStock, updatedProduct, "salesView");
            ...
            uiManager.showAlert("Error al actualizar el inventario: " + error.message, "danger");
            return false;
        }
    },
    ...
    validateState() {
        const issues = [];
        if (!Array.isArray(this.state.availableLenses)) issues.push("availableLenses no es un array");
        if (!Array.isArray(this.state.selectedLenses)) issues.push("selectedLenses no es un array");
        if (!Array.isArray(this.state.searchResults)) issues.push("searchResults no es un array");
    ...
    async findProductByBarcode(barcode) {
        ...
        if (!Array.isArray(this.state.availableLenses) || this.state.availableLenses.length === 0) return null;

        return this.state.availableLenses.find(lens => {
    ...
    handleProductUpdated(product) {
        ...
        if (Array.isArray(this.state.selectedLenses)) {
            ...
            if (selectedIndex !== -1) {
                ...
                this.state.selectedLenses[selectedIndex] = { ...product, quantity: currentQuantity };
                this.renderSelectedLenses();
            }
        }
        ...
        if (data.product) {
            ...
            const updateStockInArray = (array) => {
                ...
                if (index !== -1) {
                    ...
                    if (data.stock_surtido !== undefined) array[index].stock_surtido = data.stock_surtido;
                    return true;
                }
                return false;
            };
            ...
            if (updateStockInArray(this.state.selectedLenses)) this.renderSelectedLenses();
            if (updateStockInArray(this.state.searchResults)) this.renderSearchResults();
        }
    }
};
...
salesManager.inyectarEstilosImpresion = function() {
    ...
    styles.textContent = `
        ...
        #factura-preview-clone {
            ...
            background: white !important; padding: 25px !important; margin: 0 !important;
            font-family: 'Courier New', Consolas, monospace !important; color: #000 !important;
            border-radius: 8px !important;
        }
        #factura-preview-clone .factura-contenido { display: block !important; }
        #factura-preview-clone .pos-header { text-align: center; margin-bottom: 20px; }
        #factura-preview-clone .pos-empresa-nombre { font-weight: bold; font-size: 16px; margin-bottom: 8px; }
        #factura-preview-clone .pos-empresa-info { font-size: 13px; line-height: 1.6; }
        #factura-preview-clone .pos-linea-punteada { border-top: 1px dashed #000; margin: 15px 0; }
        #factura-preview-clone .pos-linea-solida { border-top: 2px solid #000; margin: 15px 0; }
        #factura-preview-clone .pos-titulo { text-align: center; margin: 15px 0; }
        #factura-preview-clone .pos-titulo-principal { font-weight: bold; font-size: 18px; margin-bottom: 8px; }
        #factura-preview-clone .pos-numero-factura { font-weight: bold; font-size: 16px; }
        #factura-preview-clone .pos-fecha { text-align: center; font-size: 13px; margin-bottom: 15px; }
        #factura-preview-clone .pos-cliente { margin: 15px 0; font-size: 14px; }
        #factura-preview-clone .pos-cliente-label { font-weight: bold; }
        #factura-preview-clone .flex-row { display: flex; justify-content: space-between; align-items: center; }
        #factura-preview-clone .pos-tabla-header { font-weight: bold; font-size: 14px; padding: 8px 0; border-bottom: 2px solid #000; margin-bottom: 10px; }
        #factura-preview-clone .pos-col-cant { flex: 0 0 60px; text-align: center; }
        #factura-preview-clone .pos-col-desc { flex: 1; padding: 0 15px; }
        #factura-preview-clone .pos-col-total { flex: 0 0 120px; text-align: right; }
        #factura-preview-clone .pos-producto-item { margin-bottom: 15px; font-size: 13px; }
        #factura-preview-clone .pos-producto-nombre { font-weight: bold; word-wrap: break-word; font-size: 14px; }
        #factura-preview-clone .pos-producto-descripcion { font-size: 12px; color: #555; margin-top: 4px; }
        #factura-preview-clone .pos-producto-precio { font-size: 12px; color: #666; margin-top: 4px; }
        #factura-preview-clone .pos-total-container { padding: 15px 0; border-top: 3px solid #000; border-bottom: 3px solid #000; margin: 15px 0; }
        #factura-preview-clone .pos-total-label, #factura-preview-clone .pos-total-valor { font-weight: bold; font-size: 20px; }
        #factura-preview-clone .pos-footer { text-align: center; margin-top: 20px; font-size: 13px; }
        #factura-preview-clone .pos-footer-mensaje { font-weight: bold; margin-bottom: 8px; }
        ...
        @media print {
            ...
            #factura-pos-print { display: block !important; visibility: visible !important; }
            #factura-pos-print * { visibility: visible !important; }
            .factura-contenido {
                position: relative !important; width: 100% !important; max-width: 100% !important;
                padding: 15mm !important; margin: 0 auto !important; background: white !important;
                font-family: 'Courier New', Consolas, monospace !important; font-size: 24px !important;
                line-height: 1.4 !important; color: #000 !important; box-sizing: border-box !important;
            }
            .flex-row { display: flex !important; flex-direction: row !important; justify-content: space-between !important; align-items: center !important; width: 100% !important; }
            .pos-header { text-align: center !important; margin-bottom: 8mm !important; }
            .pos-empresa-nombre { font-weight: bold !important; font-size: 32px !important; margin-bottom: 10px !important; }
            .pos-empresa-info { font-size: 24px !important; line-height: 1.5 !important; }
            .pos-linea-punteada { border-top: 2px dashed #000 !important; margin: 8mm 0 !important; height: 0 !important; width: 100% !important; }
            .pos-linea-solida { border-top: 3px solid #000 !important; margin: 8mm 0 !important; height: 0 !important; width: 100% !important; }
            .pos-titulo { text-align: center !important; margin: 8mm 0 !important; }
            .pos-titulo-principal { font-weight: bold !important; font-size: 36px !important; margin-bottom: 10px !important; }
            .pos-numero-factura { font-weight: bold !important; font-size: 32px !important; }
            .pos-fecha { text-align: center !important; font-size: 24px !important; margin-bottom: 8mm !important; line-height: 1.5 !important; }
            .pos-cliente { margin: 8mm 0 !important; font-size: 26px !important; }
            .pos-cliente-label { font-weight: bold !important; }
            .pos-tabla-header { font-weight: bold !important; font-size: 26px !important; padding: 15px 0 !important; border-bottom: 4px solid #000 !important; margin-bottom: 15px !important; }
            .pos-col-cant { flex: 0 0 100px !important; text-align: center !important; }
            .pos-col-desc { flex: 1 !important; padding: 0 20px !important; }
            .pos-col-total { flex: 0 0 200px !important; text-align: right !important; }
            .pos-producto-item { margin-bottom: 15px !important; font-size: 24px !important; page-break-inside: avoid !important; }
            .pos-producto-nombre { font-weight: bold !important; word-wrap: break-word !important; font-size: 26px !important; line-height: 1.3 !important; }
            .pos-producto-descripcion { font-size: 22px !important; color: #333 !important; margin-top: 4px !important; line-height: 1.3 !important; }
            .pos-producto-precio { font-size: 22px !important; color: #555 !important; margin-top: 4px !important; }
            .pos-total-container { padding: 20px 0 !important; border-top: 5px solid #000 !important; border-bottom: 5px solid #000 !important; margin: 15px 0 !important; page-break-inside: avoid !important; }
            .pos-total-label, .pos-total-valor { font-weight: bold !important; font-size: 38px !important; }
            .pos-footer { text-align: center !important; margin-top: 15mm !important; font-size: 24px !important; }
            .pos-footer-mensaje { font-weight: bold !important; margin-bottom: 10px !important; }
            .pos-footer-sistema { font-size: 20px !important; }
            .pos-espacio-corte { height: 30px !important; }
        }
    `;
...
salesManager.generarHTMLFacturaPOS = function(factura) {
    if (!factura) return '<div class="factura-pos-oculta">Error: No hay datos</div>';

    const fecha = new Date(factura.fechaEmision);
    const fechaStr = fecha.toLocaleDateString('es-CO', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const horaStr = fecha.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', hour12: false });
    const totalItems = factura.productos ? factura.productos.reduce((sum, p) => sum + (p.cantidad || 0), 0) : 0;
    ...
    return `
        <div id="factura-pos-print" class="factura-pos-oculta">
            <div class="factura-contenido">
                ...
                <div class="pos-fecha">Fecha: ${fechaStr}<br>Hora: ${horaStr}</div>
                <div class="pos-linea-punteada"></div>
...
salesManager.hideVistaPrevia = function() { this.cerrarVistaPrevia(); };
...
salesManager.mostrarFormularioFactura = function() {
        ...
        <div class="modal" id="factura-form-modal" style="display: flex; z-index: 1000;">
            <div class="modal-content" style="max-width: 700px; max-height: 90vh; display: flex; flex-direction: column;">
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 20px; border-bottom: 2px solid #e9ecef; background: #f8f9fa;">
                    <h3 style="margin: 0;"><i class="bi bi-file-text"></i> Datos de la Factura</h3>
    ...
    this.state.selectedLenses.forEach(lens => { lens.precioUnitario = lens.precioUnitario || 0; });
};
...
salesManager.limpiarVistaPreviaCompleta = function() {
    try {
        ...
        document.querySelectorAll('.modal[id*="factura"]').forEach(m => {
            if (m.id !== 'factura-decision-modal' && m.id !== 'factura-form-modal') m.remove();
        });
    } catch (error) {
...
salesManager.generarHTMLVistaPrevia = function(factura) {
    ...
    try {
        ...
        const horaStr = fecha.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
        const totalItems = factura.productos ? factura.productos.reduce((sum, p) => sum + (p.cantidad || 0), 0) : 0;

        return `
            <div id="factura-preview-display" style="width: 210mm; max-width: 210mm; padding: 18mm; font-family: 'Courier New', Consolas, monospace; font-size: 24px; line-height: 1.7; color: #000; background: white; margin: 0 auto; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
                <div style="text-align: center; margin-bottom: 10mm;">
                    <img src="../assets/Logo.png" alt="Logo" style="max-width: 120px; max-height: 120px; margin: 0 auto 5mm; display: block;">
                    <div style="font-weight: bold; font-size: 32px; margin-bottom: 4mm;">${factura.empresa?.nombre || 'DISTRIBUIDORA MAXI BISEL'}</div>
                ...
                <div style="text-align: center; margin: 10mm 0;">
                    <div style="font-weight: bold; font-size: 36px; margin-bottom: 4mm;">FACTURA DE VENTA</div>
                    <div style="font-weight: bold; font-size: 32px;">${factura.numeroFactura || 'N/A'}</div>
                </div>
                <div style="text-align: center; font-size: 24px; margin-bottom: 10mm; line-height: 1.8;">Fecha: ${fechaStr}<br>Hora: ${horaStr}</div>
                <div style="border-top: 3px dashed #000; margin: 10mm 0;"></div>
                    ...
                    ${factura.productos ? factura.productos.map(prod => `
                        <div style="margin-bottom: 10mm; font-size: 24px;">
                            <div style="display: flex;">
                                <div style="flex: 0 0 30mm; text-align: center; font-weight: bold; font-size: 26px;">${prod.cantidad || 0}</div>
                                <div style="flex: 1;">
                                    <div style="font-weight: bold; word-wrap: break-word; font-size: 26px; line-height: 1.5;">${prod.nombre || 'Producto'}</div>
                                    ${prod.descripcion ? `<div style="font-size: 22px; color: #333; margin-top: 3mm; line-height: 1.5;">${prod.descripcion}</div>` : ''}
                                    <div style="font-size: 22px; color: #555; margin-top: 3mm;">Precio Unit.: $${(prod.precioUnitario || 0).toLocaleString('es-CO')}</div>
                                </div>
                                <div style="flex: 0 0 55mm; text-align: right; font-weight: bold; font-size: 26px;">$${(prod.subtotal || 0).toLocaleString('es-CO')}</div>
                            </div>
                        </div>
                ...
                <div style="text-align: center; margin-top: 10mm; font-size: 24px; line-height: 1.7;">
                    <div style="font-weight: bold; margin-bottom: 4mm;">¡Gracias por su compra!</div>
                    <div style="font-size: 20px;">Software propio de Maxibisel</div>
                </div>
...
salesManager.mostrarVistaPrevia = function(factura) {
    ...
    try {
        ...
        document.body.insertAdjacentHTML('beforeend', this.generarHTMLFacturaPOS(factura));
        ...
        document.body.insertAdjacentHTML('beforeend', `
            <div class="modal" id="factura-preview-modal" style="display: flex; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); z-index: 10000; align-items: center; justify-content: center; padding: 20px;">
                <div style="background: white; border-radius: 12px; max-width: 550px; width: 100%; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
                    ...
                    <div id="preview-container" style="flex: 1; overflow-y: auto; padding: 20px; background: #e5e5e5;"></div>
...
window.addEventListener('afterprint', function() {
    const factura = document.getElementById('factura-pos-print');
    if (factura) { factura.style.display = 'none'; factura.style.left = '-9999px'; }
    const modal = document.getElementById('factura-preview-modal');
    if (modal) modal.style.display = 'flex';
});
...
salesManager.mostrarHistorialFacturas = function(facturas) {
    ...
    if (facturas.length === 0) {
        ...
        contenidoHTML = facturasOrdenadas.map(factura => {
            const fecha = new Date(factura.fechaEmision);
            const fechaStr = fecha.toLocaleString('es-CO', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' });
...
salesManager.confirmarEliminarFactura = function(facturaId) {
    ...
    document.getElementById('btn-cancelar-eliminar').onclick = () => {
        ...
        if (modal) { modal.style.animation = 'fadeOut 0.2s ease'; setTimeout(() => modal.remove(), 200); }
    };
...
salesManager.eliminarFactura = async function(facturaId) {
    try {
        ...
        if (response.success) {
            ...
            setTimeout(() => { this.verHistorialFacturas(); }, 300);
        } else {
...
salesManager.ocultarIndicadorEliminacion = function() {
    ...
    if (indicator) { indicator.style.animation = 'fadeOut 0.2s ease'; setTimeout(() => indicator.remove(), 200); }
    if (overlay) { overlay.style.animation = 'fadeOut 0.2s ease'; setTimeout(() => overlay.remove(), 200); }
};
